...
> **CANONICAL TEMPLATE.** This file is the single source of truth for the Knowledgebase (child page) structure. Both the MB Product Knowledgebase Maker and the MB Product Knowledgebase Assistant read from this file. A byte-identical mirror of this file exists at `skills/mb-product-knowledgebase-assistant/references/kb-template.md` so the Assistant has access to it when installed standalone. When editing this file, also update the mirrored copy (the plugin's regenerate script handles this automatically).
>
> **SECTIONS DEFINED IN THIS TEMPLATE ARE NEVER REMOVED from a real Knowledgebase, even if unpopulated.** The 8 numbered sections, the Human Review Status block, and the mandatory subheader table on each Core User Flow all must be present on every Knowledgebase document. If information is unknown, leave placeholder text or a TBD marker. Never delete a section. Never fold one section into another. The template may add sections or fields going forward, but the Maker and Assistant skills may only add or populate. They must never remove a section, subsection, or required field that this template defines.
